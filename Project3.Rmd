...
plot.pspline <- function(m){
  ...
  plot(m$y ~ m$x, main = 'Smoothing Plot with 95% Credible Intervals', xlab = 'x', ylab = 'y', type = 'p')
  lines(m$Fitted ~ m$x, col = 'red', lwd = 3)
  lines(ll ~ m$x, col = 'blue', lwd = 3)
  lines(ul ~ m$x, col = 'blue', lwd = 3)
  legend('bottomright', legend = c('Smoothing Curve', '95% Credible Interval Bounds'), lwd = 3, col = c('red', 'blue'), cex = .7)
  
  
  newlist <- list('ll' = ll, 'ul' = ul, 'x' = m$x)
  invisible(newlist)
